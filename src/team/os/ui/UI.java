...
import team.os.simulator._History;
import team.os.simulator._Process;
import team.os.simulator._Simulator;
...
public class UI extends JFrame {
	...
	private void addCoreSize(JTextField targetField, JTextField[] fields, int value) {
		...
		if(now + value >= 0 && using + value <= _Simulator.MAX_CORE_SIZE)
	...
	private void organizeReadyQueue() {
		...
		for(int row = 0; row < pModel.getRowCount(); row++)
			...
			processList.add(new _Process((String) pModel.getValueAt(row, 0), (int) pModel.getValueAt(row, 1), (int) pModel.getValueAt(row, 2)));
	...
	private void drawGanttChart(_History history) {
